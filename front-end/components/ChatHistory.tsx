  ...
  useEffect(() => {
    if (context?.socket) {
      context.socket.on('PrivateMessage', (payload: any) => {
        if (payload) {
          console.log(payload.sender, ' THIS IS SENDER ', localStorage.getItem('statee') , ' THIS USE WHEN I WHERE' )
...
//       console.log('this is the context ', context?.loginClick)
//       console.log('this is local ', localStorage.getItem('state'))
...
//       setStatusFriend(res.data.isOnline);}
//     };
  
//     // Call the getStatus function immediately
//     getStatus();
  
//     // Set up the interval to call getStatus every 10 seconds
//     const intervalId = setInterval(getStatus, 100);
